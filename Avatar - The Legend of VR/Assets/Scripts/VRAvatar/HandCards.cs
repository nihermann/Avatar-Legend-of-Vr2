...
namespace VRAvatar
{
    ...
    public class HandCards : MonoBehaviour
    {
        ...
        {
            // deselect if no cards are in the hand,
            // CanSelect is false (it is true when we are in selection state),
            // thumb is not on touchpad while SteamVR is active (otherwise debug won't work bc without SteamVR _thumbOnTouchpad will always be false)
            if (NumberOfCards == 0 || (!_thumbOnTouchpad && !SteamVR.active) || !CanSelect)
            {
                SetSelectedCard(-1);
                return;
            }

            // if there is only one hand card we don't need math to know which card has to be selected.
            if (NumberOfCards == 1)
            {
                SetSelectedCard(0);
                return;
